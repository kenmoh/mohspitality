...
from app.models.user_models import Subscription, User
from app.schemas.subscriptions import CreateSubscription, SubscriptionStatus, SubscriptionResponse, SubscriptionType, Updateubscription
...
async def create_staff_subscription(
    ...
    company_subscription = await db.execute(
        select(Subscription).where(Subscription.user_id ==
                                   current_user.id, Subscription.plan_name == current_user.subscriptions.plan_name)
    )
    ...
    return SubscriptionResponse(**staff_subscription.__dict__)
...
async def check_and_notify_users(db: AsyncSession):
    ...
    upcoming_expirations = await db.execute(
        select(Subscription).where(Subscription.end_date <=
                                   datetime.utcnow() + datetime.timedelta(days=3))
    )
